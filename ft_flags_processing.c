...
/*   Updated: 2021/01/05 12:56:20 by ysoroko          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_for_errors(t_list *list, va_list *v_list)
{
	...
	if (ft_str_has_other_chars(list->definer_str, ACCEPTED_CHARS))
		return (4);
	printf("4) All chars that shouldn't be present in the definer string done \n\n");
	if (ft_str_has_other_chars(list->after_dot, "0123456789*cspdiuxX%"))
		return (5);
	printf("5) All chars that shouldn't be present in the string after the '.' done \n\n");
...
int	ft_process_list(t_list *list, va_list *v_list)
{
	...
	printf("Here I am processing flags\n");
	if (printf("Error analysis: %d\n", ft_check_for_errors(list, v_list)))
		return (0);
	//AND NO '*'!!!
	if (list->type_flag == 's')
	{
